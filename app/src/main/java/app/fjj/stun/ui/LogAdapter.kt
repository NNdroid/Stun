...
import app.fjj.stun.databinding.ItemLogLineBinding
        ...
        private fun formatLogLine(line: String): Pair<SpannableString, Int> {
            ...
            when {
                lowerLine.contains("ERROR") || lowerLine.contains("FATAL") -> {
                    levelColor = "#F44336".toColorInt()
                    rowBgColor = "#1A F44336".toColorInt().let { Color.argb(30, 244, 67, 54) } // Very light red
                }
                lowerLine.contains("WARN") -> {
                    levelColor = "#FF9800".toColorInt()
                    rowBgColor = Color.TRANSPARENT
                }
                lowerLine.contains("DEBUG") -> {
                    levelColor = "#757575".toColorInt()
                    rowBgColor = Color.TRANSPARENT
                }
                lowerLine.contains("INFO") -> {
                    levelColor = "#2196F3".toColorInt()
                    rowBgColor = Color.TRANSPARENT
                }
                else -> {
